...
def scrape_baseball_names(year, follow_links=True, max_players=None):
    ...
    for table in soup.find_all('table'):
        # Look for the table with column headers including "Player"
    ...
    if not player_table:
        print("Could not find player data table on page")
        return []
    ...
    for row in player_table.find_all('tr')[1:]:  # Skip header row
        ...
        if len(cells) >= 5:
            ...
            if player_link:
                ...
                link_url = 'https://www.baseball-almanac.com' + player_link['href'] if player_link['href'].startswith('/') else player_link['href']
                ...
                if follow_links:
                    try:
                        ...
                        time.sleep(1)  # Increased from 0.2 to 1 second
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            if len(cells) >= 2:
                ...
                # Look for nickname field
                if 'nickname' in label:
                    player_data['nickname'] = value
                # Look for other potentially useful fields
...
def main():
    ...
    max_players_per_year = 5  # Start with just 5 players to test
